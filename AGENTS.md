...
- **Convenção de deep-link por tela (usada em `home.html` e replicável em qualquer link direto pra um registro):** `pendencias.html?e=<email>&p=<id>` · `clientes.html?e=<email>&c=<id>` (ou `&search=`) · `contas.html?e=<email>&id=<id>` (também aceita `e_id=`/`search=`/`q=`) · `reunioes.html?e=<email>&m=<id>` (também aceita `search=`) · `acoes-programadas.html?e=<email>&a=<id>` · `municipios.html?e=<email>&m=<nome>` (aqui `m=` é nome, não id). **`equipe.html` não tem deep-link por id** — use `?search=<nome>` como aproximação. Os nomes de parâmetro não são padronizados entre telas (`p` vs `c` vs `id` vs `m` vs `a`) — confira a tela de destino antes de assumir.
