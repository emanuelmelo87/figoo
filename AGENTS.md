...
- **Módulos Calendário e Weekly removidos (06/09):** `calendario.html` e `weekly.html` foram descontinuados e removidos do projeto. Foram retirados de `_getToolsList`, `secondaryIds` e `_getModuleId` em `figoo-ui.js` (não aparecem mais na Topbar, no menu "Mais", nem na grade de atalhos da `home.html`), do assistente de IA (`figoo-chat.js` e `auditoria-ia.html`) e o campo em `equipe.html` foi simplificado para apenas `Nome Curto *` (para apelido nos cards, presença e reuniões). **Não removido:** a regra `calendario` em `database.rules.json` e as entradas `calendario/${ek}/events` e `weekly/${ek}/w` em `_dataCollectAll` (`figoo-auth.js`) foram mantidas de propósito pelo mesmo princípio de segurança de dados antigos.
- **Autenticação Google 100% Transparente (06/09):** Usuários que autenticam via Google têm a chave de criptografia AES-GCM 256 bits derivada deterministicamente da própria conta Google (`google_auth_key_` + emailKey). O fluxo `_figGoogleOnCredential` (`figoo-auth.js`) NUNCA deve exibir janelas de `prompt(...)` ou `alert(...)` pedindo para "definir senha" ou avisando sobre "chave provisória". A autenticação do Google é a credencial soberana e a chave é inicializada de forma 100% silenciosa e automática.
- **Fim dos Formulários Legados de Setup de Senha (06/09):** Nenhuma tela (`pendencias.html`, `contas.html`, `reunioes.html`, `projetos.html`, `equipe.html`, etc.) deve exibir a view legada `setup` com campo de senha quando a sessão local expira ou não encontra a chave. Todas as telas devem fazer `window.location.href = 'index.html'` imediatamente, permitindo que o usuário reautentique com 1 clique no Google sem sustos.
- **Design System de Ícones SVG (`.figo-ic` e `.fig-dot`, 06/09):** Padronização visual em `figoo-ui.js` e em todas as telas, substituindo emojis soltos por componentes SVG vetoriais `.figo-ic` (com `aria-hidden="true"` e cores herdadas) e `.fig-dot` (bolinhas de status temáticas `.fig-dot-green`, `.fig-dot-red`, etc.).
- **Hoisting e Escopo em IIFE de Telas HTML (05/09):** Em scripts protegidos por `(function(){ 'use strict'; ... })()`, nunca declarar apenas `window.minhaFuncao = function(){...}` se ela for chamada por identificador livre (`minhaFuncao()`) dentro do script. Sempre declarar no escopo léxico (`var minhaFuncao;` ou `minhaFuncao = window.minhaFuncao = function(){...}`), garantindo disponibilidade tanto interna quanto global sem disparar `ReferenceError`.
...
- **Layout de Containers de Visão em Abas (`#view-*`, 09/09):** Em telas estruturadas por abas (como `equipe.html`), todo container de visão (`#view-*`) deve ter explicitamente no CSS `display: flex; flex-direction: column; gap: 14px; width: 100%;`. Ao alternar abas via script com `view.style.display = 'flex'`, a ausência de `flex-direction: column` força o navegador ao padrão `flex-direction: row`, quebrando os filhos diretos (ex: barra de controle/busca e grid de cards) em colunas laterais acidentais.
